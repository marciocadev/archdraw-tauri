...
import { getLambdaTriggerSource, type LambdaTriggerSource } from "./lambdaTriggerSource"

function renderTypeScriptHandler(functionName: string, triggerSource: LambdaTriggerSource): string {
  switch (triggerSource) {
    case "sqs":
      return `import type { SQSEvent, SQSBatchResponse } from "aws-lambda"
...
`
    case "sns":
      return `import type { SNSEvent } from "aws-lambda"
...
export function generateLambdaAssetFiles(resources: DiagramResources): ProjectFile[] {
  ...
  resources.lambdaFunctions.forEach((lambdaFunction, index) => {
    ...
    if (isNodejsLambdaRuntime(lambdaFunction.runtime)) {
      files.push({
        ...
        content: renderTypeScriptHandler(displayName, triggerSource),
      })
      return
    }
